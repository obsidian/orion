...
Instances of link:https://crystal-lang.org/api/HTTP/Handler.html[`HTTP::Handler`] _(a.k.a. middleware)_
can be inserted directly in your routes by using the `use` macro.
...
class MyController
  ...
  def show
    user = User.find(request.query_params["id"])
