...
A common way to interact with the router is to use standard HTTP verbs. Oak
...
[quote]
Oak will automatically add mime-type headers for requests with no Accept header and
...
You can constrain the request to a specific domain or subdomain by wrapping routes
in a `constraints` block. In this method, any routes within the block will be
...
implements the `Oak::Constraint` module.
...
1. Fork it https://github.com/<your-github-name>/oak/fork
